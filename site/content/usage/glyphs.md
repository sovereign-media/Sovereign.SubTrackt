...
reference set is involved and nothing is matched against anything; this is the pipeline stopping one
stage short of the matcher and handing you what it found.
...
Most people never need this. It is here because two things that shaped the tool were built on it,
and because it is the right instrument when an extraction is wrong in a way the report cannot
explain.
...
`--summary` suppresses the rows and leaves you with that line alone. It is the cheap "does this
file decode?" check: it exercises demux, decode, binarisation and segmentation without a reference
set anywhere in the picture, and stdout stays empty so it costs nothing to run against a
multi-gigabyte file.
...
the number of characters you can see on screen — the problem is upstream of the reference set and no
