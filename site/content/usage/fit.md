...
extraction would, and then — for each candidate set in the directory — measures how far those shapes
sit from the nearest entry in that set, averaged over all of them.

A shape that nothing in the set can account for is charged the worst possible distance, which is
what stops a set that is simply missing characters from winning by ignoring them. Lowest average
wins.

The `read` column is the same coverage figure `extract --report` prints: the fraction of shapes that
found *an* answer. It is there for context, not as the ranking key.

## It samples, and that is on purpose
...
If you have material that genuinely changes style partway — rare, but it happens — raise the sample:
...
So if the number of candidates comes out smaller than the directory looked, the run says so on
stderr rather than quietly narrowing the field. That is worth reading, because a fit that ranked
three sets out of a directory of forty is not the answer you thought you asked for.
...
months later rather than merely regrettable.
...
- **The set is right but incomplete** — no italic or bold cut, so the dialogue reads perfectly and
  the italic passages read badly. Rebuild that one set with `--italic` and `--bold`.
- **Nothing you have fits.** That is a real answer and it is one this tool is designed to be able to
  give you.
