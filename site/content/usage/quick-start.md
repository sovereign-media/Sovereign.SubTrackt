...
Four commands take you from nothing to a `.srt`. One of them you run once ever; the other three you
run per film.

Before you start, the one thing that surprises people: **SubTrackt ships with no reference set, so
until you build one every character comes back unread and you get no output.** That is deliberate
and the reasoning is on [Reference sets](/guide/reference-sets). Practically it means step 3 below
is not optional, and you need some font files.
...
`x86_64-pc-windows-msvc`, `aarch64-pc-windows-msvc`. There is no macOS build because nobody has
asked for one.
...
so the version string names both. `reference set: empty, 0 glyphs` is not a fault. It is the
shipped state, and it is why the next step exists.
...
you are not choosing anything yet, just assembling a library of candidates for `fit` to rank. This
is the one-off. You come back to it only when you meet material nothing in the library fits.
...
A few seconds even against a multi-gigabyte file, because it samples the opening rather than reading
the whole film. `-o` copies the winner out ready for the next step.

Note what the caveat under the table says, because it is the honest limit of this command and not
boilerplate: the score ranks the candidates **against each other** and cannot certify the winner.
...
time: it is a count of what the tool did and declined to do, and it is the half that makes this
different from an OCR run. [`extract`](/usage/extract) reads the fields one at a time.

Drop `--report` and `-o` and the subtitle goes to standard output, so it pipes.

## 7. Read a few cues

This is a real step, it takes a minute, and skipping it is the most common way to get a bad file
without knowing.

Open the output and read the first page. Not for accuracy — for *sanity*. What you are looking for
is the signature of a near-miss typeface, which is not gibberish but words that are almost right:
`Iater`, `rnodern`, `hoIiday`. A wrong set produces text that reads fine at a glance.

If it looks right, you are done, and the `.subtref` is now a property of that title — keep it beside
the film. If it does not, [Reading a track](/guide/reading-a-track) has the failures in order of how
...
- **It did not work, or you want to know why it is built this way** —
  [How it works](/guide/what-this-is) is the plain-language half, six short pages.
- **You want every flag** — the rest of this section is one page per command, starting with
  [`list`](/usage/list).
- **You are automating this** — [Global options](/usage/global-options) covers exit status, what
  goes to which stream, and turning the colour and progress off.
