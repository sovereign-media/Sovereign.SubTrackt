...
Ask a file what picture subtitle tracks it carries. Start here with any file you have not met
before. It is the only command that reads nothing but the container's headers.
...
| Argument | Meaning |
| :--- | :--- |
| `<INPUT>` | The file to inspect. A container, a raw `.sup` dump, or a VOBSUB `.idx`/`.sub` pair, pointed at the `.idx`. |
...
never need `--stream`. On the file above, the forced track carrying only signage and foreign
dialogue is stream `1`, and reading it means saying so.
...
fails with a demux error saying so. That failure is the right answer: you want a muxer, not this.
...
That is a **successful** run. The file opened, it was read, and it holds nothing this tool can work
with, which is a fact about the file rather than a fault. The exit status is `0`. A file that could
not be opened or parsed at all is a different case, and exits `1` with the reason on stderr, so a
script can tell the two apart without reading either message.
